<template>
  <div>
    ...
    <ion-list v-if="filteredTransactions.length > 0">
      <ion-item-sliding v-for="transaction in filteredTransactions" :key="transaction.id">
          ...
          <ion-label>
            ...
            <p>
              ...
              <template v-if="transaction.type === 'repayment'">
                {{ transaction.payer.name }}<span v-if="transaction.payer_id === currentMemberId" class="you-indicator"> (You)</span>
                → {{ getRecipientName(transaction) }}
              </template>
              <template v-else-if="transaction.type === 'income'">
                Received by {{ transaction.payer.name }}<span v-if="transaction.payer_id === currentMemberId" class="you-indicator"> (You)</span>
              </template>
              <template v-else>
                Paid by {{ transaction.payer.name }}<span v-if="transaction.payer_id === currentMemberId" class="you-indicator"> (You)</span>
              </template>
            </p>
          </ion-label>
          <ion-note slot="end" class="amount" :class="{ income: transaction.type === 'income' }">
...
import type { TransactionWithDetails, Member } from '@/types';
...
const filteredTransactions = computed(() => {
  if (filter.value === 'mine' && props.currentMemberId) {
    return props.transactions.filter((transaction) => {
      ...
      return transaction.splits.some((s) => s.member_id === props.currentMemberId);
    });
  }
